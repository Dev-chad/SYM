...
<div id="wrapper">
    ...
    <div class="clearfix">
        <a href="upload.php?category=stage" class="button red" style="float: right">업로드</a>
    </div>
    ...
    <div id="main" class="portfolio clearfix" style="width: 100%;">

        <?php

        $result = $mysqli->query("select count(*) from contents where category='stage'");
        ...
        $query = "select * from contents where category = 'stage' order by idx DESC limit $start, $viewCount";
        ...
        while ($result_arr = mysqli_fetch_array($result)) {
            if ($count % 3 != 0) {
                ?>
                <article>
                    <a href="postView.php?category=stage&postNum=<?php echo $result_arr['idx']; ?>"><img
                                src="<?php echo $result_arr['thumbnail']; ?>" alt="img" width="300" height="160"
                                style="max-height: 160px; max-width: 300px"/></a>
                    ...
                    <a href="postView.php?category=stage&postNum=<?php echo $result_arr['idx']; ?>" class="rm">Read
                        More</a>
                </article>
            <?php } else { ?>
    ...
    <div class="clearfix" align="center">
        ...
        if ($startPage != 1) { ?>
            <a href="stage.php" style="font-weight: bold; font-size: 15px; color: #0f0f0f"> 처음 </a>
            <a href="stage.php?page=<?php echo $startPage - 1; ?>"
               style="font-weight: bold; font-size: 15px; color: #0f0f0f"> << </a>
        <?php }
        for ($j = $startPage; $j <= $endPage && $j <= $totalPage; $j++) {
            if ($page == $j) { ?>
                <a style="font-weight: bold; font-size: 15px; color: #0f0f0f"> <?php echo $j; ?> </a>
            <?php } else { ?>
                <a href="stage.php?page=<?php echo $j; ?>" style="font-size: 15px;"> <?php echo $j; ?> </a>
            <?php } ?>
        <?php }
        if (($totalPage - $startPage) >= $blockCount) { ?>
            <a href="stage.php?page=<?php echo $endPage + 1; ?>"
               style="font-weight: bold; font-size: 15px; color: #0f0f0f"> >> </a>
            <a href="stage.php?page=<?php echo $totalPage; ?>"
               style="font-weight: bold; font-size: 15px; color: #0f0f0f"> 맨끝 </a>
        <?php } ?>
    </div>
    <?php } else { ?>
